    ...
    //if in the cache
      ...
      cout<<"from cache!!!!"<<endl;
        ...
        if(hasNoCache || (maxAge == -1 && hasMustRevalidate)){
          ...
          if(date_str != "" && expire_str != "" && is_expired(date_str, maxAge, expire_str)){
            getEntire(server_fd,client_connection_fd,buffer,length,cache_map);
          }else{
...
void HttpMethod::postRequest(int server_fd,
  ...
  if (end_of_headers != nullptr) return;
...
bool HttpMethod::is_expired(const std::string& date_str, int max_age, const std::string& expires_str) {
  // Parse the "Date" and "Expires" headers
  boost::posix_time::ptime date = boost::date_time::parse_delimited_time<boost::posix_time::ptime>(date_str, ' ');
  boost::posix_time::ptime expires = boost::date_time::parse_delimited_time<boost::posix_time::ptime>(expires_str, ' ');

  // Calculate the expiration time based on the "max-age" header
  boost::posix_time::ptime max_age_expiration = date + boost::posix_time::seconds(max_age);
